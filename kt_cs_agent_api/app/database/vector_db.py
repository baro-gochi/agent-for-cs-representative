...
이 모듈은 벡터 스토어의 연결과 검색을 담당합니다.
- ChromaDB 또는 PGVector 지원
- 임베딩 모델 초기화
- 벡터 DB 연결 관리
- 유사도 검색 기능
...
class VectorDBManager:
    ...
    ChromaDB 또는 PGVector와의 연결을 관리하고 검색 기능을 제공합니다.
    싱글톤 패턴으로 구현되어 있어 애플리케이션 전체에서
    하나의 인스턴스만 사용됩니다.

    Attributes:
        embedding_model: OpenAI 임베딩 모델
        vectorstore: 벡터 스토어 인스턴스 (Chroma 또는 PGVector)
        is_initialized: 초기화 완료 여부
        db_type: 사용 중인 벡터 DB 타입
    """
    ...
    def __init__(self):
        ...
        logger.info(f"VectorDBManager 인스턴스 생성됨 (DB 타입: {self.db_type}, 아직 초기화 안됨)")

    def initialize(self) -> None:
        """
        벡터 DB 연결 초기화

        임베딩 모델을 로드하고 설정에 따라 ChromaDB 또는 PGVector에 연결합니다.
        ...
        try:
            logger.info(f"벡터 DB 초기화 시작... (타입: {self.db_type})")
    ...
    def _initialize_pgvector(self) -> None:
        ...
        logger.info(f"PGVector 연결 중: {settings.POSTGRES_HOST}:{settings.POSTGRES_PORT}/{settings.POSTGRES_DB}")
        ...
        logger.info(f"PGVector 연결 완료 (컬렉션: {settings.CHROMA_COLLECTION_NAME})")
    ...
    def get_collection_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.db_type == "chroma":
                ...
                info["db_path"] = settings.CHROMA_DB_PATH
                info["document_count"] = count
            else:
                # PGVector 정보
                info["db_host"] = f"{settings.POSTGRES_HOST}:{settings.POSTGRES_PORT}"
                info["db_name"] = settings.POSTGRES_DB
                # PGVector는 직접 count 조회가 필요
                try:
                    # 간단한 검색으로 연결 확인
